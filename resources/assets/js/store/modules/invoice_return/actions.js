...
export const addInvoiceReturn = ({ commit, dispatch, state }, data) => {
  return new Promise((resolve, reject) => {
    window.axios
...
export const getInvoiceReturnNumber = ({ commit, dispatch, state }) => {
  return new Promise((resolve, reject) => {
    window.axios
      .get(`/api/v1/next-number?key=invoice_return`)
      .then((response) => {
        resolve(response)
      })
      .catch((err) => {
